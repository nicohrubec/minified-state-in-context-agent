...
def test_function():
    ...
    foo = (
        "The # character in this string should "  # This comment  # noqa
        "not result in a syntax error"
    )  # ...and this one should go away  # noqa
...
def pyminify(options, files):
    ...
    if options.pyz:
        ...
        if len(files) > 1:
            print("ERROR: The --pyz option only works with one python file at a time.")
            print("(Dependencies will be automagically included in the resulting .pyz)")
            sys.exit(1)
        # Make our .pyz:
    ...
    obfuscations = (
        options.obfuscate,
        options.obf_classes,
        options.obf_functions,
        options.obf_variables,
        options.obf_builtins,
        options.obf_import_methods,
    )
    ...
    if len(files) > 1:  # We're dealing with more than one file
        ...
        for sourcefile in files:
            ...
            if name_generator:
                obfuscate.obfuscate(
                    module, tokens, options, name_generator=name_generator, table=table
                )
        ...
        if (
            options.obfuscate
            or options.obf_classes
            or options.obf_functions
            or options.obf_variables
            or options.obf_builtins
            or options.obf_import_methods
        ):
            identifier_length = int(options.replacement_length)
            name_generator = obfuscate.obfuscation_machine(
                use_unicode=options.use_nonlatin, identifier_length=identifier_length
            )
